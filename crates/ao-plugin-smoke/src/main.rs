...
const PLUGIN_NAME: &str = "ao-plugin-smoke";
...
fn maybe_handle_manifest_flag() {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                eprintln!("ao-plugin-smoke {PLUGIN_VERSION} — STDIO plugin for AO host verification");
                eprintln!("Usage:");
                eprintln!("  ao-plugin-smoke --manifest      Print plugin manifest as JSON and exit");
                eprintln!("  ao-plugin-smoke                 Run JSON-RPC loop over stdin/stdout");
                std::process::exit(0);
            }
            _ => {}
        }
...
async fn main() -> Result<()> {
    ...
    if io::stdin().is_terminal() {
        eprintln!(
            "ao-plugin-smoke is a STDIO plugin; pipe JSON-RPC requests on stdin or pass --manifest"
        );
        std::process::exit(2);
    }
    ...
    while let Some(line) = reader.next_line().await? {
        ...
        let request: RpcRequest = match serde_json::from_str(trimmed) {
            Ok(req) => req,
            Err(error) => {
                eprintln!("[ao-plugin-smoke] invalid JSON-RPC frame: {error}");
                continue;
            }
        };
            ...
            if other.starts_with("$/") {
                ...
                Some(RpcResponse::err(
                    ...
                    RpcError {
                        ...
                        message: format!("method '{other}' not implemented by ao-plugin-smoke"),
                        data: None,
                    },
                ))
            }
...
fn handle_smoke_get(id: Option<Value>, params: Option<Value>) -> RpcResponse {
    ...
    RpcResponse::ok(
        ...
        json!({
            ...
            "description": "Synthetic subject returned by ao-plugin-smoke",
            "attributes": {
                "kind": SUBJECT_KIND,
                "echo": params,
            }
