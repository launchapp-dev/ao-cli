...
//!
//! Wraps `llm-cli-wrapper`'s ClaudeSessionBackend in the JSON-RPC 2.0 STDIO
//! plugin protocol so AO can dispatch agent/run requests through a discovered
//! plugin binary rather than calling the wrapper crate directly.
...
const PLUGIN_NAME: &str = "ao-provider-claude";
...
fn manifest() -> PluginManifest {
    PluginManifest {
        ...
        description: "Claude Code provider for AO (wraps llm-cli-wrapper claude backend)".to_string(),
...
fn handle_cli_args() {
    for arg in std::env::args().skip(1) {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                eprintln!("ao-provider-claude {PLUGIN_VERSION} — STDIO provider plugin for Claude Code");
                eprintln!("Usage:");
                eprintln!("  ao-provider-claude --manifest    Print plugin manifest as JSON and exit");
                eprintln!("  ao-provider-claude               Run JSON-RPC loop on stdin/stdout");
                std::process::exit(0);
            }
            _ => {}
        }
...
async fn main() -> Result<()> {
    ...
    if io::stdin().is_terminal() {
        eprintln!("ao-provider-claude is a STDIO plugin; pipe JSON-RPC on stdin or pass --manifest");
        std::process::exit(2);
    }

    let backend = Arc::new(ClaudeSessionBackend::new());
...
async fn handle_request(request: RpcRequest, backend: Arc<ClaudeSessionBackend>) -> Option<RpcResponse> {
            ...
            if other.starts_with("$/") {
                ...
                Some(RpcResponse::err(
                    ...
                    RpcError {
                        ...
                        message: format!("method '{other}' not implemented by ao-provider-claude"),
                        data: None,
                    },
                ))
            }
    ...
    let mut run = match run_result {
        ...
        Err(error) => {
            return error_rpc(
                ...
                RpcError {
                    code: -1002,
                    message: format!("claude session start failed: {error}"),
                    data: None,
                },
            )
        }
    ...
    match backend.terminate_session(&params.session_id).await {
        ...
        Err(error) => error_rpc(
            ...
            RpcError {
                ...
                message: format!("claude session terminate failed: {error}"),
                data: None,
            },
        ),
    }
...
fn build_session_request(params: AgentRunParams) -> SessionRequest {
    ...
    SessionRequest {
        ...
        model: params.model.unwrap_or_else(|| "claude-sonnet-4-6".to_string()),
