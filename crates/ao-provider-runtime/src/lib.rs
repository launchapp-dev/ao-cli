...
async fn handle_request<P: ProviderBackend>(
    ...
    let response = match request.method.as_str() {
        "initialize" => Some(RpcResponse::ok(
            id,
            serde_json::to_value(info.initialize_result()).expect("encode init"),
        )),
        "initialized" => None,
        ...
        "agent/resume" => {
            ...
            Some(
                handle_agent_run(id, request.params, &info, backend.clone(), stdout.clone(), resume_session).await,
            )
        }
    ...
    while let Some(event) = run.events.recv().await {
        match event {
            ...
            SessionEvent::FinalText { text } => {
                send_notification(&stdout, "agent/output", json!({ "text": text, "session_id": session_id, "final": true })).await;
