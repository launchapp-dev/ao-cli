...
pub(crate) async fn start_gemini_session(
    ...
    let selected_session_id = configured_gemini_session_id(&request).or(resume_session_id);
    ...
    tokio::spawn(async move {
        ...
        if let Err(error) = run_gemini_session(request, invocation, event_tx.clone()).await {
            let _ = event_tx
...
async fn run_gemini_session(
    ...
    let exit_code = wait_for_gemini_child(&mut child, request.timeout_secs).await?;
