...
pub(crate) async fn start_opencode_session(
    ...
    tokio::spawn(async move {
        ...
        if let Err(error) = manage_opencode_child(child, timeout_secs, event_tx.clone(), cancel_rx).await {
...
async fn manage_opencode_child(
    mut child: Child,
    ...
    let exit_code = wait_for_child(&mut child, timeout_secs, &mut cancel_rx, "opencode").await?;
