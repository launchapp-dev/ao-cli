...
When `ANIMUS_CLAUDE_BYPASS_PERMISSIONS` is unset, AO must run Claude with normal
...
| --- | --- | --- | --- |
| CLI shared runtime contract override | `crates/orchestrator-cli/src/shared/runner.rs` (`claude_bypass_permissions_enabled`) | unset `ANIMUS_CLAUDE_BYPASS_PERMISSIONS` defaults to `true` | bypass mode is silently enabled by default |
...
- Preserve explicit opt-in behavior:
  - `ANIMUS_CLAUDE_BYPASS_PERMISSIONS=true` still injects bypass mode.
- Preserve explicit disable behavior:
  - `ANIMUS_CLAUDE_BYPASS_PERMISSIONS=false` keeps normal permissions.
...
## Acceptance Criteria
- `AC-01`: In `shared/runner.rs`, unset `ANIMUS_CLAUDE_BYPASS_PERMISSIONS` no
  longer injects `--permission-mode bypassPermissions`.
- `AC-02`: In daemon runtime support, unset
  `ANIMUS_CLAUDE_BYPASS_PERMISSIONS` no longer injects bypass mode.
- `AC-03`: Explicit truthy `ANIMUS_CLAUDE_BYPASS_PERMISSIONS` still injects bypass
  mode in both code paths.
- `AC-04`: Explicit falsy `ANIMUS_CLAUDE_BYPASS_PERMISSIONS` continues to disable
