...
impl AoMcpServer {
    ...
    #[tool(
        name = "animus.requirements.get",
        description = "Get a requirement by its ID. Purpose: View full requirement details including title, description, priority, status, and linked tasks. Prerequisites: None. Example: {\"id\": \"REQ-001\"}. Sequencing: Use after animus.requirements.list to get details, or before animus.task.create to link new tasks.",
    ...
    async fn ao_requirements_get(&self, params: Parameters<RequirementGetInput>) -> Result<CallToolResult, McpError> {
        ...
        self.run_tool("animus.requirements.get", args, input.project_root).await
    }

    #[tool(
        name = "animus.requirements.create",
        description = "Create a requirement in AO. Purpose: Add a new requirement with structured metadata and acceptance criteria. Prerequisites: None. Example: {\"title\": \"Offline mode\", \"priority\": \"must\", \"acceptance_criterion\": [\"Sync resumes after reconnect\"]}. Sequencing: Use animus.requirements.get or animus.requirements.update to inspect or refine the new requirement, and animus.task.create to create derived tasks.",
    ...
    async fn ao_requirements_create(
        ...
        self.run_tool("animus.requirements.create", args, input.project_root).await
    }

    #[tool(
        name = "animus.requirements.update",
        description = "Update a requirement in AO. Purpose: Modify requirement title, description, priority, status, acceptance criteria, and linked tasks. Prerequisites: Requirement must exist. Example: {\"id\": \"REQ-001\", \"status\": \"in-progress\", \"acceptance_criterion\": [\"Exports CSV\"]}. Sequencing: Use animus.requirements.get first to inspect current state, or animus.requirements.refine for AI-assisted refinement.",
    ...
    async fn ao_requirements_update(
        ...
        self.run_tool("animus.requirements.update", args, input.project_root).await
    }

    #[tool(
        name = "animus.requirements.delete",
        description = "Delete a requirement from AO. Purpose: Remove a requirement that is obsolete or created in error. Prerequisites: Requirement must exist. Example: {\"id\": \"REQ-099\"}. Sequencing: Use animus.requirements.get first to verify the requirement, and ensure linked tasks have been handled before deletion.",
    ...
    async fn ao_requirements_delete(
        ...
        self.run_tool("animus.requirements.delete", args, input.project_root).await
    }

    #[tool(
        name = "animus.requirements.refine",
        description = "Refine requirements in AO. Purpose: Improve one or more requirements, optionally with AI assistance, before planning or task derivation. Prerequisites: Requirements should exist. Example: {\"id\": [\"REQ-001\"], \"focus\": \"tighten acceptance criteria\"}. Sequencing: Use animus.requirements.get or animus.requirements.list to choose targets first, then animus.requirements.update or animus.task.create to apply follow-up work.",
    ...
    async fn ao_requirements_refine(
        ...
        self.run_tool("animus.requirements.refine", args, input.project_root).await
    }
}
