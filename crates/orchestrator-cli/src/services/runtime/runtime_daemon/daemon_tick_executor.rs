...
pub(super) fn full_project_tick_driver() -> FullProjectTickDriver {
    HookBackedProjectTickDriver::new(FullProjectTickHooks {
...
#[cfg(test)]
pub(super) struct FullProjectTickHooks {
    pub(super) schedule_process_manager: ProcessManager,
}

#[cfg(test)]
#[async_trait::async_trait(?Send)]
impl ProjectTickHooks for FullProjectTickHooks {
    ...
    async fn recover_orphaned_running_workflows(
        ...
        let _ = recover_orphaned_running_workflows_with_active_ids(hub, root, &active_subject_ids)
            .await;
        Ok(())
    }
    ...
    async fn dispatch_ready_tasks(
        ...
        run_ready_task_workflows_for_project(hub, root, limit).await
    }
...
#[async_trait::async_trait(?Send)]
impl ProjectTickHooks for SlimProjectTickHooks<'_> {
    fn build_hub(&mut self, root: &str) -> Result<Arc<dyn ServiceHub>> {
        Ok(Arc::new(FileServiceHub::new(root)?))
    }
    ...
    async fn dispatch_ready_tasks(
        ...
        dispatch_ready_tasks_via_runner(hub, root, self.process_manager, limit).await
    }
