...
//!
//! Drives the real `animus` binary against the bundled `ao-plugin-smoke`,
//! `ao-provider-claude`, and `ao-provider-codex` plugin binaries via the
//! `ao plugin {list,info,call,ping}` CLI. Locks in:
//!
//! - Discovery via `AO_PLUGIN_PATH`
...
fn ensure_plugin_binary(name: &str) -> Result<PathBuf> {
    ...
    if !candidate.exists() {
        return Err(anyhow!(
            "{name} not built; run `cargo build -p {name}` before running plugin_contract_e2e tests"
        ));
    }
    Ok(candidate)
}
...
fn run_plugin_command(args: &[&str]) -> Result<Value> {
    ...
    harness.run_json_ok_with_env(args, &[("AO_PLUGIN_PATH", plugin_path.as_str())])
}

#[test]
fn plugin_list_discovers_smoke_and_provider_plugins() -> Result<()> {
    ensure_plugin_binary("ao-plugin-smoke")?;
    ensure_plugin_binary("ao-provider-claude")?;
    ensure_plugin_binary("ao-provider-codex")?;
    ...
    for required in ["ao-plugin-smoke", "ao-provider-claude", "ao-provider-codex"] {
    ...
    for plugin in plugins {
        for required in ["name", "version", "plugin_kind", "description", "source", "path"] {
            assert!(
                plugin.get(required).is_some(),
                "discovered plugin entry should include `{required}`: {plugin}"
            );
        }
    }
...
fn plugin_info_completes_handshake_for_smoke() -> Result<()> {
    ensure_plugin_binary("ao-plugin-smoke")?;
    let response = run_plugin_command(&["plugin", "info", "--name", "ao-plugin-smoke"])?;
...
fn plugin_info_completes_handshake_for_claude_provider() -> Result<()> {
    ensure_plugin_binary("ao-provider-claude")?;
    let response = run_plugin_command(&["plugin", "info", "--name", "ao-provider-claude"])?;
    let init = response.pointer("/data/initialize").context("initialize block missing")?;
    assert_eq!(
        init.pointer("/plugin_info/plugin_kind").and_then(Value::as_str),
        Some("provider"),
...
fn plugin_info_completes_handshake_for_codex_provider() -> Result<()> {
    ensure_plugin_binary("ao-provider-codex")?;
    let response = run_plugin_command(&["plugin", "info", "--name", "ao-provider-codex"])?;
    let init = response.pointer("/data/initialize").context("initialize block missing")?;
    assert_eq!(
        init.pointer("/plugin_info/plugin_kind").and_then(Value::as_str),
        Some("provider"),
...
fn plugin_ping_round_trips() -> Result<()> {
    ensure_plugin_binary("ao-plugin-smoke")?;
    let response = run_plugin_command(&["plugin", "ping", "--name", "ao-plugin-smoke"])?;
...
fn plugin_info_for_unknown_plugin_returns_not_found() -> Result<()> {
    let plugin_path = plugin_path_env()?;
    let harness = CliHarness::new()?;
    let (payload, status) = harness
        .run_json_err_with_exit_and_env(
            &["plugin", "info", "--name", "ao-plugin-does-not-exist"],
            &[("AO_PLUGIN_PATH", plugin_path.as_str())],
        )?;
