...
//!
//! v0.4.9: scrubs secret-shaped values from `orchestrator_logging::LogEntry`
//! fields before the daemon hands them to the active
//! [`crate::log_storage::LogStorageDispatch`] sink. Patterns match keys
//! such as `api_key`, `apikey`, `password`, `token`, `secret`, and
//! `authorization` followed by `:` or `=` and a value. The value is
//! replaced with `***REDACTED***` while the key is preserved so operators
//! still see the *shape* of what was logged.
//!
...
//!
//! The redactor is intentionally `&mut LogEntry` rather than a wrapping
//! transform so the call site does one in-place mutation and proceeds —
//! avoiding allocations on the hot log write path.

use std::sync::OnceLock;

use orchestrator_logging::LogEntry;
use regex::Regex;
