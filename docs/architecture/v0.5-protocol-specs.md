...
`memory_mcp_stdio_command` is optional and exists for plugins that need to
spawn the Animus memory MCP sidecar using the exact host binary path instead of
guessing a sibling executable location. Plugins that do not recognize this
...
pub struct QueueReorderRequest {
    /// Single entry id to move. Mutually exclusive with `entry_ids`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Optional relative placement strategy. `before` / `after` require `anchor_id`;
    /// omitted means "move to the front in the provided order".
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub position: Option<QueueReorderPosition>,
    /// Anchor entry id for relative moves.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
**Hold / release / drop / mark_assigned / completion all return `QueueMutationResponse`.** Reorder returns `QueueReorderResponse` with the count of entries whose position changed. Control callers must provide exactly one of `id` or `entry_ids`; sending both is invalid. `position=before|after` also requires `anchor_id`.
...
Lift the equivalent queue behavior coverage from the current in-tree queue implementation:
...
2. **`InitializeParams.init_extensions` map** — opaque per-extension blobs the host may pass on initialize. Used by v0.5 to send `project_binding` (see Common Conventions) plus optional host hints such as `memory_mcp_stdio_command`. Plugins ignore extensions they don't recognize. Forward-compatible.
