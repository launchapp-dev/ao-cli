...
- `ANIMUS_PROVIDER_DISABLE_PLUGIN=1` — Forces `SessionBackendResolver` to
  skip installed provider plugins and dispatch through the in-tree
  (claude/codex/gemini/opencode/oai-runner) backends only. Useful when a
  freshly-installed provider plugin is misbehaving and you need the bundled
  implementation back without uninstalling. Daemon restart needed to
  re-enable.
