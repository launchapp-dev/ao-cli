...
In v0.3.x the provider crates (Claude, Codex, Gemini, OpenAI-compatible, OpenCode) lived in the Animus core workspace at `crates/ao-provider-*/`. In v0.4.0 they ship from independent GitHub repositories:
...
If you have nothing to migrate, skip this section. If you want to try it:
...
Subject backend plugins ship from their own GitHub repositories (`animus-subject-linear`, `animus-subject-jira`, etc.) — see the protocol design doc for current support and roadmap.
