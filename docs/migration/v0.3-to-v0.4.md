...
The 19 skills that previously shipped baked into the `animus` binary (`code-review`, `implementation`, `debugging`, `refactoring`, `unit-testing`, etc.) are now distributed as the `animus.core-skills` installable pack. The pack auto-installs during `animus init`.
