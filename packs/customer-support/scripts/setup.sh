#!/usr/bin/env bash
#
# packs/customer-support/scripts/setup.sh
#
# Idempotent setup for the Customer Support reference pack.
# Installs required plugins and seeds the local ticket inbox.
#
...
INBOX_DIR="${PROJECT_ROOT}/tickets/inbox"
WORKFLOW_OVERLAY_DIR="${PROJECT_ROOT}/.animus/workflows"

echo "==> Customer Support pack setup"
...
if [ ! -f "${WORKFLOW_OVERLAY_DIR}/triage-ticket.yaml" ]; then
  cp "${PACK_DIR}/workflows/triage-ticket.yaml" "${WORKFLOW_OVERLAY_DIR}/"
  echo "    Copied triage-ticket.yaml"
else
  echo "    triage-ticket.yaml already present — leaving it untouched"
fi

# 3. Seed the ticket inbox. Split the bundled sample-tickets.md into one
#    file per ticket so each can be dispatched independently. Idempotent:
#    skips tickets that already exist on disk.
echo
echo "==> Seeding ticket inbox at ${INBOX_DIR}"
mkdir -p "${INBOX_DIR}"

SAMPLE_BUNDLE="${PACK_DIR}/subjects/sample-tickets.md"
if [ ! -f "${SAMPLE_BUNDLE}" ]; then
  echo "WARN: ${SAMPLE_BUNDLE} missing — skipping ticket seeding" >&2
else
  # Idempotent: skip any TKT-* file that already exists on disk so a
  ...
  awk -v outdir="${INBOX_DIR}" '
    ...
    /^## TKT-[0-9]+:/ {
      if (out) close(out)
      out = ""
      # Extract ticket id (TKT-1001) from "## TKT-1001: ..."
      match($0, /TKT-[0-9]+/)
    ...
    /^---$/ {
      # Section separators in the bundle — do not write to per-ticket files
      next
    }
    out { print >> out }
  ' "${SAMPLE_BUNDLE}"
  echo "    Per-ticket files in ${INBOX_DIR}/ (existing files left untouched):"
  ls "${INBOX_DIR}"/TKT-*.md 2>/dev/null | sed 's/^/      /'
fi
...
directory we just seeded, regardless of cwd):
  ...
  4. Run the workflow synchronously against a single ticket. The
     workflow run CLI currently requires --task-id, --requirement-id,
     or --title (no first-class --subject-id for arbitrary kinds yet),
     so we pass the title AND the ticket body via --description so the
     classify/draft phases actually see the ticket content:
...
To customize categories, tone, models, or wire to a real helpdesk, see
packs/customer-support/docs/customizing.md.
EOF
